function [fbank,filt_ctrs,ctr_shift,filt_len] = gen_scale_fbank_light(ctr_min,ctr_max,...
...
% samprate: spectral sampling rate
% nfft: number of samples in the transform domain
%
% Outputs: 
% fbank: cell array containing constant-Q/variabale-Q scale filters
...
addParameter(func_inputs,'filt_name','gabor_fourier',@ischar);
...
% valued so the high-/lowpass filters are replaced by filters with wider
% supports (Tukey)
for i = [1,n_bpass+2]
    len_temp = filt_len(i);
