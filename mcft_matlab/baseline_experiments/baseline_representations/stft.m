function [Sout,Fout,Tout,Pout] = stft(x,winlen,wintype,ovp,nfft,fs,Opt)
...
for i=1:NumBlockZ
    xw = W.* xz((i-1)*hop+1:(i-1)*hop+winlen);    
    XX=fft(xw,nfft);
    XX_trun=XX(1:Fel_max);
    
    SFmax(:,i)=XX_trun;
    PFmax(:,i)=(1/fs)*((abs(SFmax(:,i)).^2)/Wnorm2);   
end
...
%% Plot the Spectrogram and return the results
