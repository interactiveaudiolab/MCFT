function [src_est,sdr,sir,sar]=separability_cft(src_trg,mix,stft_params,cft_params,ibm_thr)

% This function measures the separability of the CFT domain
%
% Inputs:
...
for i=1:num_sources
    ...
    for j=1:num_sources-1
        interf_temp = interf_temp + src_cft{interf_nums(j)};
    end
...
for i=1:num_thr
    for j=1:num_sources
        ...
        src_masked{i,j} = ibm_temp.*mix_cft;        
    end
end

% compute the ICFT of all estimated sources
src_est_stft = cell(num_thr,num_sources);

stft_dims=size(mix_stft);
icft_params=struct('spec_size',stft_dims,'hop_size',cft_params.hop_size);
        
for i=1:num_thr
    for j=1:num_sources
        est_cft_temp = src_masked{i,j};
        est_stft_temp = inv_cft(est_cft_temp,icft_params);
        src_est_stft{i,j} = est_stft_temp;
    end
end
...
for i=1:num_thr
    ...
    for j=1:num_sources
        est_stft_temp = src_est_stft{i,j};
